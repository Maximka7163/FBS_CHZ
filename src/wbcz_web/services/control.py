...
class ControlService:
 ...
 def run(self,import_id:str,user_id:int,mode:OperationMode,event_ids:list[str]|None=None)->dict:
  ...
  if event_ids is None:selected=list(dict.fromkeys(allowed))
  else:
   ...
   wanted=set(requested);selected=list(dict.fromkeys(x for x in allowed if x in wanted))
  ...
  for eid in selected:
   ...
   outcome=sequence_outcome_for_event(self.imports,eid)
   source="wb-sequence-policy" if outcome is not None else "mock-v04-deterministic"
   if outcome is None:
   ...
   self.db.add(CheckRecord(run_id=run.id,event_id=eid,source=source,snapshot=asdict(snapshot) if snapshot else None,decision=outcome.decision.value,reason=outcome.reason,error=outcome.error));outcomes.append(outcome)
