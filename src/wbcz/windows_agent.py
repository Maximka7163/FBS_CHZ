...
class AgentJob:
    ...
    def validate(self) -> None:
        ...
        if self.job_type in {AgentJobType.LK_RECEIPT, AgentJobType.LP_RETURN}:
            ...
            if not self.cises or len(self.cises) > 1000:
                raise AgentSecurityError("CIS_CHECK requires 1..1000 cises")
            if any(not isinstance(cis, str) or not cis for cis in self.cises):
                raise AgentSecurityError("CIS_CHECK contains invalid CIS")
...
class ProductionAgentTrueApiTransport:
    ...
    def cises_info(self, cises: tuple[str, ...], *, bearer_token: str) -> Any:
        if not cises or len(cises) > 1000:
            raise AgentSecurityError("cises/info batch must contain 1..1000 cises")
