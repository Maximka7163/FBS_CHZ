...
APP_VERSION = "0.5.1"
EXPECTED_ALEMBIC_HEAD = "0016_m15_production_hardening"
ARCHIVE_FORMAT = "sellari-marking-p0-rc1-v1"
...
OPERATIONAL_FILES = (
    ...
    "docs/M15_FIRST_REAL_WRITE_RUNBOOK.md",
    "docs/P0_RC1_RUNTIME_RUNBOOK.md",
    "docs/P0_RC1_RECONCILIATION_AUDIT.md",
    "scripts/m15_backup_contract.py",
    "scripts/m15_release_manifest.py",
...
FORBIDDEN_SUFFIXES = {
    ".xlsx", ".xls", ".sqlite", ".sqlite3", ".db", ".pem", ".key", ".p12", ".pfx", ".pyc",
}
SECRET_MARKERS = (
    b"-----BEGIN PRIVATE KEY-----",
...
def _copy_file(root: Path, bundle: Path, relative: str) -> None:
    ...
    if not source.is_file():
        raise FileNotFoundError(f"required RC1 file missing: {relative}")
...
def _copy_tree(root: Path, bundle: Path, relative: str) -> None:
    ...
    if not source.is_dir():
        raise FileNotFoundError(f"required RC1 directory missing: {relative}")
...
def _copy_frontend_dist(source_root: Path, bundle: Path) -> None:
    ...
    for path in sorted(p for p in dist.rglob("*") if p.is_file()):
        ...
        shutil.copyfile(path, target)
    joined = b"\n".join(
        p.read_bytes() for p in sorted((bundle / "frontend" / "dist").rglob("*")) if p.is_file()
    )
...
def _write_metadata(
    ...
    *,
    ...
    metadata = {
        ...
        "agent_identity_model": "participant-bound-enrollment-dpapi",
        "legacy_global_agent_bootstrap": False,
        "private_key_server_side": False,
        "pin_server_side": False,
...
def _scan(bundle: Path) -> None:
    for path in sorted(bundle.rglob("*")):
        ...
        if any(part in FORBIDDEN_PARTS for part in rel.parts):
            raise ValueError(f"forbidden path in RC1 bundle: {rel.as_posix()}")
        if path.name in FORBIDDEN_FILENAMES or path.suffix.lower() in FORBIDDEN_SUFFIXES:
            raise ValueError(f"forbidden file in RC1 bundle: {rel.as_posix()}")
        data = path.read_bytes()
        for marker in SECRET_MARKERS:
            if marker in data:
                raise ValueError(f"secret-like marker in RC1 bundle: {rel.as_posix()}")

    compose = (bundle / "docker-compose.prod.yml").read_text(encoding="utf-8")
    if 'WBCZ_TRUE_API_WRITE_ENABLED: "false"' not in compose:
        raise ValueError("RC1 compose must hard-disable True API writes")
    if 'WBCZ_AGENT_LEGACY_BOOTSTRAP_ENABLED: "false"' not in compose:
        raise ValueError("RC1 compose must disable legacy global agent bootstrap")
    if not (bundle / f"migrations/versions/{EXPECTED_ALEMBIC_HEAD}.py").is_file():
        raise ValueError("M15 migration head missing from RC1 bundle")
    required = (
        "src/wbcz/document_assembler.py",
        "src/wbcz/windows_agent.py",
        "src/wbcz/windows_agent_runtime.py",
        "src/wbcz/agent_identity.py",
        "src/wbcz_web/api/agent_routes.py",
        "src/wbcz_web/api/enrollment_routes.py",
        "src/wbcz_web/services/document_orchestration.py",
        "src/wbcz_web/services/agent_orchestration.py",
        "src/wbcz_web/worker.py",
    )
    for relative in required:
        if not (bundle / relative).is_file():
            raise ValueError(f"required P0/M15 runtime surface missing: {relative}")
    if (bundle / "src/wbcz_ui").exists():
        raise ValueError("desktop/live True API package must not be shipped in VPS RC1")
        ...
        with tarfile.open(tar_path, "w", format=tarfile.PAX_FORMAT) as tar:
            ...
            for path in sorted(bundle.rglob("*"), key=lambda p: p.relative_to(bundle).as_posix()):
                _add(
                    tar,
                    path,
                    PurePosixPath(bundle.name) / PurePosixPath(path.relative_to(bundle).as_posix()),
                    mtime,
                )
...
def build_bundle(
    *,
    ...
    if source_branch != "release/p0-rc1":
        raise ValueError("P0 RC1 runtime bundle must be built from release/p0-rc1")
    mtime, normalized = _timestamp(build_timestamp_utc)
    output_dir.mkdir(parents=True, exist_ok=True)
    prefix = f"sellari-marking-p0-rc1-{APP_VERSION}-{source_sha[:12]}"
    with tempfile.TemporaryDirectory(prefix="wbcz-p0-rc1-") as temp:
        bundle = Path(temp) / prefix
        bundle.mkdir()
        for relative in ROOT_FILES:
            _copy_file(source_root, bundle, relative)
        for relative in SOURCE_DIRS:
            _copy_tree(source_root, bundle, relative)
        ...
        _write_metadata(
            ...
            source_branch=source_branch,
            build_timestamp_utc=normalized,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build deterministic P0 RC1 VPS runtime archive")
