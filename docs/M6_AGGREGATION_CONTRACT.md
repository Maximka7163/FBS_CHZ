...
Accepted M5 is not rewritten. Aggregate-aware hardening is wired into M5 evidence/reconciliation: fresh precondition snapshots persist `parent`, package type and direct-child evidence; if the accepted M5 operation had a parent relation, business reconciliation does not become `RECONCILED` until a fresh M6 relation observation is supplied. DISTANCE and nested introductions require the documented disaggregation readback/history; LP_RETURN distinguishes aggregate return from nested-child return; WRITE_OFF KITU/ATK requires expected disaggregation while WRITE_OFF KIN remains MANUAL_REVIEW; LK_REMARK KIN uses INTRODUCED/RETIRED-specific confirmation; LK_RECEIPT_CANCEL requires relation re-read but never assumes graph restoration from restored statuses alone.
