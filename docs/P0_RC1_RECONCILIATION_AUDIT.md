...
| --- | --- | --- | --- |
| `734c9139fa141cd49bc765a26afc3f25530acc8b` | expose P0 runtime config fail-closed | **REQUIRED_PORT** | M15 supersedes the old global machine-token/write wiring, but the accepted P0 document assembler still requires organisation metadata. RC1 semantically ports only `WBCZ_ORGANISATION_TYPE`, `WBCZ_ACTIVITY_FIAS_ID`, `WBCZ_ACTIVITY_KPP` and `WBCZ_REMOTE_SALE_RETURN_PAID` into the M15 web/worker runtime; write remains hard-off and legacy bootstrap remains disabled. |
